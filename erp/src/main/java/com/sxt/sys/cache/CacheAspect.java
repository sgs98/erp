...
public class CacheAspect {
    ...
    //声明一个缓存容器
    public Map<String,Object> CACHE_CONTAINER=new HashMap<>();
    ...
    * */
    ...
    public  Object cacheDeptGet(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        //从缓存里面取
        Dept res1 = (Dept) CACHE_CONTAINER.get(CACHE_DEPT_PROFIX + object);

        if(res1!=null){
            log.info("已从缓存里面找到部门对象" + CACHE_DEPT_PROFIX + object);
            return res1;
        }else {
            Dept res2 = (Dept) joinPoint.proceed();//执行目标方法
    ...
    public  Object cacheDeptUpdate(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if(isSuccess){
            Dept dept= (Dept) CACHE_CONTAINER.get(CACHE_DEPT_PROFIX + deptVo);
            if(dept==null){
                dept=new Dept();
